...
Purpose of this File: Control file that defines the array_count function used in manager.asm. This file takes the user defined
array and its number of elements to loop and validates the number of integers. The count is returned to the caller in manager.asm. 

File name: array_count.cpp
Language: C++
Compile: g++ -c -m64 -Wall -std=c++17 -fno-pie -no-pie -o array_count.o array_count.cpp
...
extern "C" long array_count(long arr[], long arr_size);

//Prints the contents of the array, up to arr_size, determined by the fill asm module
long array_count(long arr[], long arr_size) {
  long count = 0;
